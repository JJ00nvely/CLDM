...
class CLDM(ModelMixin, ConfigMixin):
    def __init__(self,latent_dim=256, num_heads=8, dropout_r=0.,num_layers=6,activation='gelu',
                 cond_emb_size=256, use_temp = False, video_length=16, backbone_name ='resnet50', freeze_extractor=True):
        super().__init__()
        ...
        logging.info('Loading Feature Extractor: %s', self.backbone_name)


        self.extractor = ImageFeatureExtractor(backbone_name=self.backbone_name)
        logging.info('Freeze Extractor: %s', self.freeze_extractor)
        if self.freeze_extractor == True:
    ...
    def forward(self, noisy_sample, timesteps): # src : image embedding
        ...
        ftrs = self.extractor(image.clone()) # B 256 23 23 -> Visual Feature Embedding : )

        ftrs= rearrange(ftrs, 'b c w h -> b (w h) c' )
    ...
    #     # 처음 Noise 를 추가해서 loc emb 하고 size emb 어떻게 제대로 나누었는지 box 에다가 Noise 를 한꺼번에 주는지 아니면 x,y,w,h 를 다 따뤄 nOISE 를 주는지 체크하기
        ...
        output_noise = self.decode(enc_output) # B, 1, 4
        return output_noise
